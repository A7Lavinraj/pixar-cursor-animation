...
// Keep track of previous star.
let previousStarPosition = { x: 0, y: 0 };

// Star class for encapsulation.
class Star {
  ...
  constructor(x, y) {
    ...
    this.partical.innerHTML = `<svg viewBox="0 0 576 512"><path fill=${
  ...
  // Calculate distance between previous and current star.
  distance() {
    return Math.sqrt(
  ...
  // Creating star by appending it to body.
  create() {
    // Star will be created at certain gap.
    if (this.distance() >= 50) {
      previousStarPosition = { x: this.x, y: this.y };
      document.body.appendChild(this.partical);

      // Destroying star after a short period.
      setTimeout(() => {
        document.body.removeChild(this.partical);
      }, 1000);
    }

    // Adding background glow effect to cursor.
